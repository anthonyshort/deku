...
export let setAttribute = curry(function (el, previousValues, value, name) {
  let previousValue = previousValues[name]
  ...
  if (events[name]) {
    if (previousValue) removeEvent(el, events[name], previousValue)
    addEvent(el, events[name], value)
    return
  }
  switch (name) {
    case 'checked':
...
export let removeAttribute = curry(function (el, nextValues, name, value) {
  if (name in nextValues) {
    return
  }
  let nextValue = nextValues[name]
...
export let insertAtIndex = curry(function (parent, index, el) {
...
export let appendTo = curry(function (container, el) {
...
export let replaceElement = curry(function (oldEl, newEl) {
...
export function createElement (type, attributes = {}) {
  ...
  forEach(attributes, setAttribute(el, {}))
