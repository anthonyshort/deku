...
const {NOT_MODIFIED, CREATED, MOVED, DELETED} = diff
...
function patchChildren (owner, prev, next, parentEl, path) {
  ...
  let left = V.keys(prev.children)
  let right = V.keys(next.children)
  ...
  forEach(diff(left, right, 'key'), change => {
    let {state, oldIndex, newIndex, item} = change
    let leftNode = item.node
    let rightNode = rightKeys[item.key]
    let nativeEl = childNodes[oldIndex]
    let newPath = `${path}.${newIndex}`
    let oldPath = `${path}.${oldIndex}`

    switch (state) {
      case MOVED:
        var {entity, el} = patch(owner, leftNode, rightNode, nativeEl, newPath)
        $.insertAtIndex(parentEl, newIndex, el)
        break
      case DELETED:
        var entity = removeComponentChildren(owner, oldPath)
        $.removeElement(nativeEl)
        break
      case CREATED:
        ...
        break
      case NOT_MODIFIED:
        var {entity} = patch(owner, leftNode, rightNode, nativeEl, oldPath)
        break
    }
  ...
  return {entity: owner, el: parentEl}
}
