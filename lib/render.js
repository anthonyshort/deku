...
function render (app, container, opts) {
  ...
  function diffChildren (path, entityId, prev, next, el) {
    ...
    var keylessChildren = zip(prev.children, next.children).filter(keyFilter)
    var j = -1

    function keyMapReducer(acc, child) {
      if (child.key) acc.key = child
      return acc
    }

    function keyFilter(child) {
      var left = child[0]
      var right = child[1]
      if (left && left.key) return false
      if (right && right.key) return false
      return true
    }
    ...
    for (var i = 0; i < keylessChildren.length; i++) {
      var leftNode = keylessChildren[i][0]
      var rightNode = keylessChildren[i][1]
