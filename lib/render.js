...
function render(app, container, opts) {
  ...
  app.on('unmount', removeCurrentElement);
  app.on('mount', onmount);
  app.on('source', updateSources);
  ...
  function teardown() {
    ...
    app.off('unmount', removeCurrentElement);
    app.off('mount', onmount);
    app.off('source', updateSources);
  }
  ...
  function onSourceChange(name, data) {
  ...
  function renderEntity(entity) {
    ...
    var result = component.render(entity.props, entity.state, setState(entity));
  ...
  function render() {
    ...
    if (!currentNativeElement) {
      ...
      patch(rootId, currentElement, app.element, rootElement);
    } else {
      for (var entityId in children[rootId]) {
  ...
  function unmountChildren(entityId) {
    for (var path in children[entityId]) {
      unmount(children[entityId][path]);
    }
  }
  ...
  function diffChildren(path, entityId, prev, next, el) {
    ...
    for (var i = 0; i < children.length; i++) {
      ...
      // this is a new node.
      if (left == null) {
        var childEl = toNative(entityId, childPath, right);
      ...
      if (right == null) {
        removeElement(entityId, childPath, el.childNodes[j])
        j = j - 1;
        continue;
      }

      diffNode(childPath, entityId, left, right, el.childNodes[j]);
    }
  }
  ...
  function removeElement(entityId, path, el) {
    ...
    if (childrenByPath[path]) {
      ...
      for (var childPath in childrenByPath) {
        if (childPath === path || isWithinPath(path, childPath)) {
          unmount(childrenByPath[childPath]);
    ...
    children[entityId] = omit(children, removals);
  ...
  function updateEntityProps(entityId, props) {
    var entity = entities[entityId]
    entity.pendingProps = props;
    entity.dirty = true;
    invalidate();
  }
  ...
  function removeAllEvents(entity) {
    keypath.del(handlers, [entity.id]);
  }
