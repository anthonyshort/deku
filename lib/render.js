...
function render(app, container, opts) {
  ...
  function unmountEntity(entityId) {
    ...
    entities = omit(entities, entityId);
    children = omit(children, entityId);
  }
   ...
   * optional: Boolean
   *
   * Instead of throwing any real errors, we'll just show warnings
   * in the console.
   *
  ...
  function validateProps(props, rules){
    ...
    each(rules, function(name, options){
      ...
      if (!optional && value == null) {
        errors.push('Missing prop named: ' + name);
      }
      if (options.type && type(value) !== options.type) {
        errors.push('Invalid type for prop named: ' + name);
      }
      if (options.expects && options.expects.indexOf(value) < 0) {
        errors.push('Invalid value for prop named: ' + name + '. Must be one of ' + options.expects.toString());
      }
    });

    // Now check for props that haven't been defined
    for (var key in props) {
      if (!rules[key]) errors.push('Unexpected prop named: ' + key);
    }
