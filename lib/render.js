...
function patchChildren (owner, prev, next, el, path) {
  ...
  let prevChildren = reduce(prev.children, V.keyMapReducer, {})
  let nextChildren = reduce(next.children, V.keyMapReducer, {})
  ...
  while (prevKeys[i] || nextKeys[i]) {
    ...
    if (!nextKey) {
      ...
      let leftNode = prevChildren[prevKey]
      let rightNode = nextChildren[nextKey]
      let prevChild = leftNode.vnode
      let nextChild = rightNode.vnode
      let {entity, el} = patch(owner, prevChild, nextChild, childNodes[leftNode.index], `${path}.${leftNode.index}`)
      owner = entity
