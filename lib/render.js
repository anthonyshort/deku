...
export default function render (app, container) {
  ...
  let onmount = vnode => tree = render(tree, vnode)
  let onunmount = () => removeNativeElement()
  ...
  if (app.element) tree = render(tree, app.element)
  ...
  function render (tree, vnode) {
    let {nativeElement, virtualElement} = tree

    if (!nativeElement) {
      nativeElement = toNative(tree, '0', vnode)
      removeAllChildren(container)
      container.appendChild(nativeElement)
    } else {
      nativeElement = patch(tree, virtualElement, vnode, nativeElement)
    }

    return { ...tree, nativeElement, virtualElement: vnode }
  }
