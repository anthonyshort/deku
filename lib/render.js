...
function render(app, container, options) {
  var frameId;
  var isRendering;
  var rendered = {};
  rendered.remove = remove;
  rendered.update = render;
  addNativeEventListeners();
  app.on('unmount', onunmount);
  app.on('mount', onmount);
  reset();
  if (app.mounted) onmount(app.mounted);
  ...
  /**
   * Whenever a new node is mounted onto the app.
   */

  function onmount(virtualElement) {
    if (rendered.root) {
      patch(null, currentTree, nextTree, rendered.el);
    } else {
      rendered.el = toNative('root', '0', virtualElement);
      container.appendChild(rendered.el);
    }
  }

  /**
   * Whenever a node is unmounted, we'll remove the
  ...
  function unmount(entity) {
    ...
    removeAllHandlers(entity);

    // Remove the entity without touching the original
    // object so that it retains fast propertiesin v8.
    app.entities = omit(app.entities, entity.id);
  }
  ...
  function renderEntity(entity) {
    ...
    var result = component.render(entity.props, entity.state);
  ...
  /**
   * Update the DOM to the latest version.
   *
   * @api public
   */
  ...
  function update(id) {
    ...
    if (!shouldUpdate(entity)) return updateChildEntities(id);
  ...
  function updateChildEntities(id) {
  ...
  function remove() {
    ...
    removeElement('root', '0', rendered.el);
    reset();
  ...
  function diffAttributes(prev, next, el, entity, path) {
    ...
    for (var name in nextAttrs) {
      ...
      if (!(name in prevAttrs) || prevAttrs[name] !== value) {
        setAttribute(entity.id, path, el, name, value);
      }
    }
    ...
    for (var oldName in prevAttrs) {
      if (!(oldName in nextAttrs)) {
        removeAttribute(entity.id, path, el, oldName);
      }
    }
  }
  ...
  function diffComponent(path, entity, prev, next, el) {
    if (next.component !== prev.component) {
      ...
      updateEntityProps(entity, path, next);
    }
  }
  ...
  function removeElement(entityId, path, el) {
    ...
    app.entities[entityId].children = omit(children, removals);
  ...
  function updateEntityProps(entity, path, vnode) {
  ...
  function setAttribute(entityId, path, el, name, value) {
    if (entityId && events[name] != null) {
      addHandler(entityId, path, events[name], value);
      return;
    }
    switch (name) {
      case 'value':
  ...
  function trigger(name, entity, args) {
    if (typeof entity.component[name] === 'function') {
      args.push(updateEntityState.bind(null, entity.id)); // last arg is `send`
  ...
  function updateEntityState(entityId, nextState) {
    var entity = rendered.entities[entityId];
    ...
    sources.forEach(function(source){
      ...
      function update(data) {
        ...
        for (var id in entities) {
          ...
          var changes = {};
          changes[prop] = data;
          var props = assign({}, entity.props);
          props = assign(props, changes);
          entity.pendingProps = assign(entity.pendingProps, props);
          entity.dirty = true;
          invalidate();
        };
      }
  ...
  function setDefaults(entity) {
    ...
    sources.forEach(function(source){
      var name = map[source];
      if (null != entity.pendingProps[name]) return;
      entity.pendingProps[name] = app.store[source]; // get latest value plugged into global store
    });
  }
  ...
  function handleEvent(event) {
    ...
    while (target && target.__entity__ === entityId) {
      var fn = keypath.get(app.handlers, [entityId, target.__path__, eventType]);
      if (fn) {
  ...
  function addHandler(entityId, path, eventType, fn) {
    keypath.set(app.handlers, [entityId, path, eventType], throttle(function(e){
      var entity = app.entities[entityId];
      fn.call(null, e, entity.props, entity.state, updateEntityState.bind(null, entityId));
    }));
  }
  ...
  function removeHandler(entityId, path, eventType) {
    keypath.del(app.handlers, [entityId, path, eventType]);
  }
  ...
  function removeAllHandlers(entity) {
    keypath.del(app.handlers, [entity.id]);
  }
