...
function patchChildren (owner, prev, next, parentEl, path) {
  ...
  let prevChildren = V.keys(prev.children)
  let nextChildren = V.keys(next.children)
  let prevKeys = Object.keys(prevChildren)
  let nextKeys = Object.keys(nextChildren)
  let i = 0

  while (prevKeys[i] || nextKeys[i]) {
    var key = prevKeys[i] || nextKeys[i]
    var leftNode = prevChildren[key]
    var rightNode = nextChildren[key]
    i++
  ...
  forEach(positions, function (childEl, newPosition) {
    var target = parentEl.childNodes[newPosition]
