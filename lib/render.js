...
function render (app, container, opts) {
  ...
  function diffAttributes (prev, next, el, entityId, path) {
    ...
    forEach(nextAttrs, function (value, name) {
      var previousValue = prevAttrs[name]
      if (events[name] || !(name in prevAttrs) || previousValue !== value) {
        setAttribute(el, name, value, previousValue)
      }
    })
  ...
  function setAttribute (el, name, value, previousValue) {
    if (!value) {
      removeAttribute(el, name, previousValue)
      return
    }
    if (events[name]) {
  ...
  function removeAttribute (el, name, value) {
    if (events[name] && value) {
      removeEvent(el, events[name], value)
      return
    }
    switch (name) {
      case 'checked':
