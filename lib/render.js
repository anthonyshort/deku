...
function render (app, container, opts) {
  ...
  /**
   * Update all of the paths on the DOM nodes
   *
   * @param {String} entityId
   */

  function updateElementPaths (entityId) {
    if (entityId) {
      var entity = entities[entityId]
      update(entity.nativeElement, '0')
    } else {
      update(currentNativeElement, '0')
    }

    function update(el, prefix) {
      for (var index in el.childNodes) {
        var childEl = el.childNodes[index]
        if (!childEl.__path__) continue
        var path = prefix + '.' + index
        childEl.__path__ = path
        if (childEl.childNodes.length) {
          update(childEl, path)
        }
      }
    }
  ...
  function diffChildren (path, entityId, prev, next, el) {
    ...
    var currentHandlers = assign({}, handlers[entityId])
    ...
    if (hasKeys) {
      ...
      for (var key in rightKeys) {
        ...
        for (var childPath in currentChildren) {
          ...
          } else if (isWithinPath(leftPath, childPath)) {
            var updatedPath = childPath.replace(leftPath, rightPath)
            delete children[entityId][childPath]
            children[entityId][updatedPath] = childId
          }
        }

        // Now the event handlers
        for (var handlerPath in currentHandlers) {
          var eventHandlers = currentHandlers[handlerPath]
          if (leftPath === handlerPath) {
            delete handlers[entityId][handlerPath]
            handlers[entityId][rightPath] = eventHandlers
          } else if (isWithinPath(leftPath, handlerPath)) {
            var updatedPath = path.replace(leftPath, rightPath)
            delete handlers[entityId][path]
            handlers[entityId][updatedPath] = eventHandlers
          }
        }
      }
