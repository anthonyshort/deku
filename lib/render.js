...
function render (app, container, opts) {
  ...
  function toNativeElement (entityId, path, vnode) {
    ...
    if (!options.pooling || !canPool(tagName)) {
      if (isSVGElement(tagName)) {
        el = document.createElementNS('SVG', tagName)
      } else {
  ...
  function setAttribute (entityId, path, el, name, value) {
    ...
    switch (name) {
      ...
      case isSVGAttribute(name):
        el.setAttributeNS('SVG', name, value)
        break
      default:
  ...
  function getPool (tagName) {
    ...
    if (!pool) {
      var poolOpts = isSVGElement(tagName) ?
        { namespace: svgNamespace, tagName: tagName } :
        { tagName: tagName }
...
function isSVGElement(name) {
  return svgElements.indexOf(name) !== -1
}
...
function isSVGAttribute(attr) {
  return svgAttributes.indexOf(attr) !== -1
}
