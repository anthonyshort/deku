...
HTMLRenderer.prototype.createElement = function(entityId, path, vnode){
...
HTMLRenderer.prototype.removeElement = function(entityId, path, el) {
  var self = this;
...
HTMLRenderer.prototype.replaceElement = function(entityId, path, el, vnode) {
  // TODO: use this

  var newEl = this.createElement(entityId, path, vnode);
  var child = this.children[entityId][path];

  replaceElement(oldEl, newEl);

  // unmount any nested components.

  if (child) {
    this.unmount(child);
  } else {
    this.unmountAllWithinPath(entityId, path);
  }
