...
function rendering(app) {
  ...
  function mount(renderData) {
    ...
    renderData.children = {};
    app.renders[renderData.id] = renderData;
    ...
    var nativeElement = renderData.layers.main.nativeElement;
  ...
  function unmount(renderData) {
    var el = renderData.layers.main.nativeElement;
    if (!el) return;
    ...
    delete app.renders[renderData.id];
  }
  ...
  function update(id) {
    var renderData = app.renders[id];
    if (!shouldUpdate(renderData)) return updateChildren(id);

    var currentTree = renderData.layers.main.virtualElement;
    ...
    renderData.layers.main.virtualElement = nextTree;
    updateEvents(renderData);
    updateChildren(id);
  ...
  function updateEvents(renderData) {
    ...
    var nodes = tree(renderData.layers.main.virtualElement).nodes;
  ...
  function updateChildren(id) {
    var children = app.renders[id].children;
  ...
  function remove() {
    ...
    // element with it.
    var el = app.root.layers.main.nativeElement;
    var id = app.root.id;
  ...
  /**
   * Remove all of the child renders of an renderData
   *
   * @param {RenderData} renderData
   */

  function unmountChildren(renderData) {
    var renders = app.renders;
    var children = renderData.children;
    for (var path in children) {
      var childId = children[path];
      unmount(renders[childId]);
    }
  }
  ...
  function toNative(renderDataId, path, vnode) {
    var renderData = app.renders[renderDataId];
  ...
  function removeElement(renderDataId, path, el) {
    var children = app.renders[renderDataId].children;
    var renders = shallowCopy(app.renders);
    var renderData = renders[renderDataId];

    // If the path points to a component we should use that
    // components element instead, because it might have moved it.
    if (children[path]) {
      var childId = children[path];
      var child = renders[childId];
      el = child.layers.main.nativeElement;
      unmount(child);
      ...
      for (var childPath in children) {
        if (childPath === path || isWithinPath(path, childPath)) {
          unmount(renders[children[childPath]]);
    ...
    walk(el, function(node){
      ...
      var parent = renders[node.__renderData__];
  ...
  function replaceElement(renderData, path, el, vnode) {
    ...
    for (var id in renders) {
      var renderData = renders[id];
      if (renderData.layers.main.nativeElement === el) {
        renderData.layers.main.nativeElement = newEl;
      }
    }
  }
  ...
  function updateRenderData(renderData, path, vnode) {
    ...
    var renderData = app.renders[childId];
  ...
  function register(renderData) {
    var component = renderData.component;
    // all renders for this component type.
    var renders = component.renders = component.renders || {};
    // add renderData to component list
    renders[renderData.id] = renderData;
