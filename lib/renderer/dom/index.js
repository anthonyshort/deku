...
function render(app, container) {
  ...
  /**
   * This stores all the instance data might
  ...
  /**
   * Force updating the DOM
   */

  mounted.update = function() {
  ...
  function onmount(node) {
    store.root = new Entity(component, props);
    var el = mount(store.root);
  ...
  function addHandler(entityId, path, eventType, fn) {
    keypath.set(app.handlers, [entityId, path, eventType], throttle(function(e){
      var entity = app.entities[entityId];
      fn.call(null, e, entity.props, entity.state);
    }));
  }
