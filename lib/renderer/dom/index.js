...
Renderer.prototype.updateEvents = function(entity) {
  ...
  each(currentTree, function(path, node){
...
Renderer.prototype.createElement = function(entityId, path, vnode){
  ...
  if (vnode.type === 'element') {
    ...
    // Create a pool so we can re-use DOM nodes instead of creating
    // new ones each time we need them.
    if (!pool) {
    ...
    // Remove all of the old attributes from the node. We need to
    // do this because we're reusing nodes.
    for (var i = el.attributes.length - 1; i >= 0; i--) {
      var name = el.attributes[i];
      el.removeAttribute(name);
    }

    // Remove all children from the node in case we're re-using
    // one of them
    while (el.firstChild) {
...
Renderer.prototype.removeElement = function(entityId, path, el) {
  ...
  if (el.nodeType !== 3) {
    walk(el, function(node){
      var pool = self.pools[node.tagName];
      if (pool) pool.push(node);
    });
  }
};
