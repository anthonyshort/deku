...
function dom(world) {
   ...
   */
   ...
   function diffNode(current, next, context){
    ...
    if (current.type !== next.type) {
      return replaceElement(context.entity, context.path, context.el, next);
    }
  ...
  function diffComponent(previous, current, context){
    if (current.component !== previous.component) {
      replaceElement(context.entity, context.path, context.el, current);
    } else {
      updateEntity(context.entity, context.path, current);
    }
  }
  ...
  function diffElement(previous, current, context){
    ...
    if (current.tagName !== previous.tagName) {
      return replaceElement(context.entity, context.path, context.el, current);
    }
