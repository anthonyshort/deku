...
Renderer.prototype.createElement = function(entityId, path, vnode){
  var parent = this.entities[entityId];
  ...
  if (vnode.type === 'element') {
    ...
    // If pooling is enabled we can just grab an element
    // from the pool instead of wasting time creating one.
    if (parent.option('disablePooling')) {
...
Renderer.prototype.removeElement = function(entityId, path, el) {
  ...
  if (!parent.option('disablePooling') && el.tagName) {
