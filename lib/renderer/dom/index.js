...
function dom(world) {
  ...
  function init(path, component, properties, container) {
    ...
    world.children = {};
    world.pools = {};
    container.appendChild(mount(world.root));
  }
  ...
  function update(entity) {
    ...
    if (!entity.shouldUpdate()) return next();
  ...
  function mount(entity) {
    ...
    var el = createElement(entity.id, '0', current);
  ...
  function createElement(entityId, path, vnode) {
    ...
    if (vnode.type === 'element') {
      ...
      for (var i = 0, n = children.length; i < n; i++) {
        var childEl = createElement(entityId, path + '.' + i, children[i]);
  ...
  function replaceElement(entityId, path, el, vnode) {
    ...
    var newEl = createElement(entityId, path, vnode);
  ...
  function diffChildren(previous, current, context){
    ...
    for (var i = 0; i < children.length; i++) {
      ...
      // this is a new node.
      if (left == null) {
        var childEl = createElement(context.entity.id, childPath, right) ;
