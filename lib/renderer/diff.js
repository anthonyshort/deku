...
function diffChildren(rendered, previous, current, el) {
  ...
  children.forEach(function(item, index){
    ...
    if (right == null) {
      removeComponents(rendered, left);
...
function replaceNode(rendered, previous, current, el) {
  ...
  removeComponents(rendered, previous);
...
function removeComponents(rendered, node){
  // text nodes can't have children
  if (node.type === 'text') return;

  // remove a child component
  if (node.type === 'component') {
    var path = rendered.previous.getPath(node);
    rendered.children[path].remove();
    delete rendered.children[path];
  }

  // recursively remove components
  if (node.children) {
    node.children.forEach(function(child){
      removeComponents(rendered, child);
    });
  }
}
