...
exports.diffElement = function(previous, current, el){
  ...
  // Add/remove events
  this.diffEvents(previous, current, el);
...
/**
 * Diff the events of the virtual node
 *
 * @param {Node} previous
 * @param {Node} current
...
exports.diffEvents = function(previous, current, el){
  var currentEvents = current.events;
  var previousEvents = previous.events;
  var previousPath = this.previous.getPath(previous);
  var currentPath = this.current.getPath(current);

  // remove old events
  for (var name in previousEvents) {
    this.unbind(previousPath, name);
  }

  // add new events
  for (var name in currentEvents) {
    this.bind(currentPath, name, currentEvents[name]);
  }
...
exports.replaceNode = function(previous, current, el){
  ...
  if (isRoot) {
    ...
    if (container.children.length !== 0) {
      var target = container.childNodes[previous.index];
