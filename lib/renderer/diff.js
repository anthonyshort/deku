...
exports.diffEvents = function(current, el){
  var path = this.current.getPath(current);

  for (var eventType in current.events) {
    this.delegate(path, eventType, current.events[eventType]);
  }
};
