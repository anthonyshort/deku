...
/**
 * Force update the component. This invalidates the component
...
ComponentRenderer.prototype.update = function(){
  ...
  if (!this.dirty || this.lifecycleState === 'updating') return;
  ...
  // pre-update.
  this.trigger('beforeUpdate', [ this.state, this.props, nextState, nextProps ]);

  // the props and state haven't changed.
  if (!nextProps && !nextState) return done();

  // props and state are the same.
  if (equal(nextProps, this.props) && equal(nextState, this.state)) return done();
  ...
  // post-update.
  this.trigger('afterUpdate', [ this.state, this.props, previousState, previousProps ]);
...
ComponentRenderer.prototype._afterMount = function(){
  this.trigger('afterMount', [this.el]);
