...
Entity.prototype.updateEvents = function() {
  ...
  // Reapply them
  for (var path in this.current.nodes) {
    var node = this.current.nodes[path];
    for (var eventType in node.events) {
      var callback = node.events[eventType];
      this.events.bind(this.id, path, eventType, function(e){
        callback.call(self.component, e, self.state, self.props);
      });
    }
  }
};