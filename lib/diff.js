...
function compare(current, updated, patch) {
  ...
  if (updated.isText() && current.isText() && currentElement.data !== updatedElement.data) {
    patch.add(currentPath, function(el){
  ...
  if (updatedElement.tagName !== currentElement.tagName || updatedElement.tagName !== currentElement.tagName) {
    patch.add(currentPath, function(el){
      el.parentNode.replaceChild(updated.render(), el);
    });
    return;
  }
  ...
  zip(currentElement.children, updatedElement.children)
